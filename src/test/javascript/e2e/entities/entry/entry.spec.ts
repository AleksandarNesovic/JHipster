...
describe('Entry e2e test', () => {
    ...
    it('should load Entries', async () => {
        ...
        expect(await entryComponentsPage.getTitle()).toMatch(/firstApp.entry.home.title/);
    });

    it('should load create Entry page', async () => {
        await entryComponentsPage.clickOnCreateButton();
        entryUpdatePage = new EntryUpdatePage();
        expect(await entryUpdatePage.getPageTitle()).toMatch(/firstApp.entry.home.createOrEditLabel/);
    ...
    /* it('should delete last Entry', async () => {
        ...
        expect(await entryDeleteDialog.getDialogTitle())
            .toMatch(/firstApp.entry.delete.question/);
